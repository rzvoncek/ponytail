...
| IBM Bob | `.bob/rules/ponytail.md`, `.bob/custom_modes.yaml`, `hooks/bob-hooks.json`, `skills/`, `AGENTS.md` | Bob auto-loads `AGENTS.md` from the workspace root as always-on context (zero setup). `.bob/rules/ponytail.md` provides workspace rules (or copy to `~/.bob/rules/` globally). `.bob/custom_modes.yaml` adds a dedicated selectable Ponytail persona mode for workspace-local use; for global availability copy it to `~/.bob/settings/custom_modes.yaml` (Bob Shell 2.x reads global modes from `~/.bob/settings/`, not `~/.bob/`). Skills can be loaded from `.bob/skills/` or globally at `~/.bob/skills/`. Lifecycle hooks via `hooks/bob-hooks.json` template register `UserPromptSubmit` in `.bob/settings.json` or `~/.bob/settings/settings.json` for dynamic mode switching. |
